...
CREATE EXTENSION istore;
SELECT bigistore_seed(2,5,8::bigint);
SELECT bigistore_seed(2,5,NULL::bigint);
SELECT bigistore_seed(2,5,0::bigint);
SELECT bigistore_seed(2,2,8::bigint);
SELECT bigistore_seed(2,0,8::bigint);
ROLLBACK;
BEGIN;
...
CREATE EXTENSION istore;
SELECT bigistore_val_larger('1=>1,2=>1,3=>3'::bigistore, '1=>2,3=>1,4=>1');
ROLLBACK;
BEGIN;
-- bigistore functions should merge istores by smaller keys;
-- ./spec/istore/functions_plain_spec.rb:298;
CREATE EXTENSION istore;
SELECT bigistore_val_smaller('1=>1,2=>1,3=>3'::bigistore, '1=>2,3=>1,4=>1');
ROLLBACK;
BEGIN;
...
CREATE EXTENSION istore;
SELECT bigistore_length(bigistore(ARRAY[]::integer[],ARRAY[]::integer[]));
ROLLBACK;
BEGIN;
-- bigistore functions should return length of non-empty istores;
-- ./spec/istore/functions_plain_spec.rb:362;
CREATE EXTENSION istore;
SELECT bigistore_length(bigistore(ARRAY[1],ARRAY[1]));
SELECT bigistore_length(bigistore(ARRAY[1,2,3],ARRAY[1,2,3]));
SELECT bigistore_length('-2147483647 => 10, -10 => -9223372036854775807, 0 => 5, 10 => 9223372036854775806, 2147483647 => 10'::bigistore);
ROLLBACK;
BEGIN;
-- bigistore functions should convert istore to json;
-- ./spec/istore/functions_plain_spec.rb:368;
CREATE EXTENSION istore;
SELECT bigistore_to_json('5=>50, 7=>70, 9=>90'::bigistore);
